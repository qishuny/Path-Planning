...
public:
  ...
  void addVertex(int id, Node* newNode){
    vertices[id] = newNode;
  }
  
  vector<int> Neighborhood(double* tempConfig){
    vector<int> output;
    for (int i=0; i < vertices.size(); ++i) {
        double dist = calc_distance(vertices[i]->angle_rad.data(), tempConfig, numofDOFS);
        if (dist <= RADIUS) {
            if (dist != 0) {
                // don't add itself
                output.push_back(i);
